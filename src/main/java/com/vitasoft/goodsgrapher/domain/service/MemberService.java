...
//        return metadataRepository.findAllByRegId(memberId).stream()
//                .map(metadata -> new GetMetadataDto(metadata, articleFileRepository.countByArticleIdAndRegIdAndIsDeleted(metadata.getModelSeq(), metadata.getRegId(), "0")))
